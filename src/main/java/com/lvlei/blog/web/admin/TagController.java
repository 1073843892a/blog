...
public class TagController {
    ...
    public String tags(@Valid Tag tag, BindingResult result,RedirectAttributes attributes){
        Tag t1=tagService.finTagByName(tag.getName());
        if(t1==null){
            tagService.saveTag(tag);
        }
        else {
            result.rejectValue("name","nameError","不能添加同名的tag");
        }
        ...
        else attributes.addFlashAttribute("message","新增失败");
    ...
    public String editInput(@Valid Tag tag,BindingResult result,@PathVariable Long id,RedirectAttributes attributes){
        ...
        else result.rejectValue("name","nameError","不能添加同名的tag");
        if(result.hasErrors())return "admin/tags-input";
